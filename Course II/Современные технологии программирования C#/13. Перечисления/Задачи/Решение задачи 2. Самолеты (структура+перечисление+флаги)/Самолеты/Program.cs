...
namespace Самолеты
{
    ...
    class Program
    {
        static void Main()
        {
            Plane pl1 = new Plane("Ту-154", "Аэрофлот", Owner.Владелец, 
                Day.понедельник | Day.среда | Day.воскресенье);  ////
            Plane pl2 = new Plane("Ту-134", "Аэрофлот", Owner.Аренда, 
                Day.понедельник | Day.суббота); 
            Plane pl3 = new Plane("Ил-96/1", "Сибирь", Owner.Лизинг, 
                Day.понедельник| Day.вторник | Day.пятница);     ////
            Plane pl4 = new Plane("Ил-96/2", "Сибирь", Owner.Владелец, 
                Day.понедельник | Day.воскресенье);     ////
            Plane pl5 = new Plane("Ил-96/3", "Сибирь", Owner.Лизинг, 
                Day.суббота | Day.воскресенье | Day.вторник);     ////
            Plane pl6 = new Plane(pl1);
            ...
            for (int i = 0; i < lst.Count; i++)
                if ((lst[i].days & Day.воскресенье) == Day.воскресенье)
                {
                    Plane pp;
                    pp = lst[i];
                    pp.days = lst[i].days ^ Day.воскресенье;
                    lst[i] = pp;
                }
