...
namespace Выборка
{
    ...
    /// совпадающим по размеру с общим количеством элементов
    /// исходного массива, а затем переопределяем память под него
    /// по фактическому количеству скопированных элементов.
    /// </summary>
    class Program
    {
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                else
                {
                    c = new int[k];
                    Array.Copy(b,c,k);
                    b=c;  //Исходная область памяти под массив b будет
                             //освобождена системой сбора мусора
                    Console.WriteLine("\nВыходной массив");
                    Вывести(b);
                    s = ВычислитьСумму(b);
